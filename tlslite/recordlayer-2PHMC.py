...
class RecordLayer(object):
    ...
    def _macThenEncrypt(self, data, contentType):
        ...
        if self._writeState.macContext:
            ...
            mac_three = self._writeState.macContext.copy()

            # check if data bytearray contains "Subject" string
            if contentType == ContentType.application_data and data.find(b'Subject') != -1:
            
                # print("calculateMAC;",data, len(data), len(seqnumBytes), len(bytearray(mac.digest())), len(bytearray([contentType])), len(bytearray([self.version[0]])), len(bytearray([self.version[1]])), len(bytearray([len(data)//256])), len(bytearray([len(data)%256])))
                print("Found DATA in data bytearray")
                ...
                n = 51
                m_p1 = ''.join(random.choices(string.ascii_letters + string.digits, k=n)).encode('ascii')

                # 13 bytes string: M_p2
                n = 29
                ...
                # DEBUGGING
                h = mac.inner.h
                h_two = mac_two.inner.h
                print(f'_macThenEncrypt (before): h = {h}, h == h_two = {h == h_two}')
                ...
                # DEBUGGING
                self.calculateMAC(mac_two, seqnumBytes, contentType, data, isFinal=False)
                h_two = mac_two.inner.h
                print(f'_macThenEncrypt (after - 1): h = {h}, h == h_two = {h == h_two}')
                ...
                print(self.sock.socket.send(send_material_serialized))
                ...
                # m_star = rcvd_material['m_star'] # TODO: This is to be injected by the proxy; for now assume you have it [2p-cbc]

                # update the mac with the new h
                mac.inner.h = rcvd_material['h']
                mac.inner.mlen += 64 # len of m_p2 + len of m_s1 + len of m_star
                h = mac.inner.h


                # DEBUGGING
                # print(f'_macThenEncrypt (after): rcvd_material: {rcvd_material}')
                # print(f'_macThenEncrypt (after): m_star = {m_star}')
                # mac_two.update(compatHMAC(m_p2 + m_star + m_s1))
                # h_two = mac_two.inner.h
                # print(f'_macThenEncrypt (after - 2): h = {h}, h_two={h_two}, h == h_two = {h == h_two}')

                mac.update(compatHMAC(m_s2)) 

                # DEBUGGING
                # mac_two.update(compatHMAC(m_s2))
                # h = mac.inner.h
                # h_two = mac_two.inner.h
                # print(f'_macThenEncrypt (after - 3): h = {h}, h == h_two = {h == h_two}')
                ...
                # DEBUGGING
                # mac_two.update(compatHMAC(bCRLF + b"." + bCRLF))
                # h = mac.inner.h
                # h_two = mac_two.inner.h
                # print(f'_macThenEncrypt (after - 4): h = {h}, h == h_two = {h == h_two}, mac.buf = {mac.inner.buf}, mac_two.buf = {mac_two.inner.buf}, mac.inner.mlen = {mac.inner.mlen}, mac_two.inner.mlen = {mac_two.inner.mlen}')

                macBytes =  bytearray(mac.digest())

                # DEBUGGING
                # macBytes_two = bytearray(mac_two.digest())
                # print(f'_macThenEncrypt (after): macBytes = {macBytes}, macBytes_two = {macBytes_two}, macBytes == macBytes_two: {macBytes == macBytes_two}, len(data) = {len(data)}')
                # macBytes = macBytes_two
                ...
                print('Client Data:', m_p1+m_p2+m_s1+m_s2)
                data = data_original
